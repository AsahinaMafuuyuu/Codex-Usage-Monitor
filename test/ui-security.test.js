...
test("the static UI does not require inline styles under the self-only CSP", async () => {
  ...
  assert.match(script, /data-request-inspector-tab="context_delta"/u);
  assert.match(script, /\/requests\/\$\{encodeURIComponent\(requestId\)\}\/context-delta/u);
  assert.match(script, /nextTab === "context_delta" && !state\.requestInspector\.contextDelta\.payload/u);
  assert.match(script, /CONTEXT DELTA & CACHE CORRELATION/u);
  assert.match(script, /Exact provider cache-key behavior and causal attribution are unavailable/u);
  assert.match(script, /Canonical Accounting Delta/u);
  assert.match(script, /Cache Hit Rate/u);
  assert.match(script, /pp/u);
  assert.match(script, /Removed \/ Superseded Context/u);
  assert.match(script, /Cache Correlation Evidence/u);
  assert.match(script, /Correlation only/u);
  assert.match(script, /Exact provider cache causality unavailable/u);
  assert.doesNotMatch(script, /Root Cause Found|Cache key changed|This caused the cache miss/iu);
  ...
  assert.match(styles, /\.request-context-delta\s*\{/u);
  assert.match(styles, /\.request-delta-accounting-grid/u);
  assert.match(styles, /\.request-delta-details/u);
